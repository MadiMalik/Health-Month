...
export function buildSummary(entries) {
  ...
  if (highPainDays.length >= 3) {
    flags.push(`Pain ≥7/10 on ${highPainDays.length} day${highPainDays.length > 1 ? "s" : ""}. Consider discussing frequency and management with your clinician.`);
  }
  if (medianFirst != null && medianSecond != null && medianSecond > medianFirst) {
    flags.push(`Median pain increased (${medianFirst} → ${medianSecond}) across the month.`);
  }
  if (periodDays.length >= 3 && painOnPeriod.length && painOffPeriod.length) {
    ...
    if (medPeriod != null && medNon != null && medPeriod > medNon) {
      flags.push(`Higher pain on period days (median ${medPeriod}) than non-period days (median ${medNon}).`);
    }
  }
  if (sleepLT6.length >= 3) {
    const rate = pct(highPainOnLowSleep.length, sleepLT6.length);
    if (rate >= 40) {
      flags.push(`On nights with <6h sleep, pain ≥6/10 occurred ${rate}% of the time.`);
    }
  }
  if (lowHydration.length >= 3) {
    const rateH = pct(highPainOnLowHydration.length, lowHydration.length);
    if (rateH >= 40) {
      flags.push(`On days with ≤3 glasses of water, pain ≥6/10 occurred ${rateH}% of the time.`);
    }
  }
  if (bpStats) {
    flags.push(
      `BP ranged ${bpStats.sysMin}/${bpStats.diaMin}–${bpStats.sysMax}/${bpStats.diaMax} (avg ~${bpStats.sysAvg}/${bpStats.diaAvg} mmHg). Consider discussing how to track BP appropriately.`
    );
  }
  ...
  if (highPainDays.length) {
    prompts.push(
      `Over the last month I had ${highPainDays.length} day${highPainDays.length > 1 ? "s" : ""} with pain ≥7/10. What should I track next week to help assess possible causes?`
    );
  }
  if (medianFirst != null && medianSecond != null && medianSecond > medianFirst) {
    prompts.push(`My pain seems to trend higher in the second half of the month. Is there a likely reason, and what patterns should I monitor?`);
  }
  if (periodDays.length >= 3 && painOnPeriod.length && painOffPeriod.length) {
    prompts.push(`Pain looks higher on period days. Is this expected for my situation, and should I track anything more specific during that time?`);
  }
  if (sleepLT6.length >= 3) {
    prompts.push(`On nights with <6h sleep, pain was often higher. Could sleep be a factor, and what changes should I try?`);
  }
  if (prompts.length < 3) {
...
function buildTextSummary(range, counts, flags, prompts) {
  const lines = [];
  lines.push(`Health Month — Visit Summary (Education only; not medical advice)`);
  lines.push(`Range: ${range}`);
  lines.push(`Entries: ${counts.daysLogged} day${counts.daysLogged > 1 ? "s" : ""} logged`);
  lines.push("");
  if (flags.length) {
  ...
  lines.push("");
  lines.push("Notes");
  lines.push("• This summary organizes personal observations only. It is not medical advice.");
  return lines.join("\n");
}